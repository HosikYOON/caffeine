...
[작성 가이드]
1. 단순한 소비 줄이기 조언이 아니라, **"어떤 카테고리가 인기 있으니 어떤 마케팅을 하면 좋겠다"**는 식의 관리자 관점 조언을 작성하세요.
2. 예시: "식비와 외식 카테고리 거래가 급증했습니다. 주말 외식 쿠폰 프로모션을 진행하여 트래픽을 더 확보하는 전략이 유효해 보입니다."
3. 가독성을 위해 **번호 매기기(1. 2. 3.)** 형식을 반드시 사용하고, **핵심 키워드는 굵게(`**키워드**`) 표시**하세요.
4. **키워드 뒤에는 줄바꿈**이 들어가도록 의도해 주세요 (프론트엔드에서 처리 예정).
5. 예시 포맷: `1. **핵심 전략:** 상세 내용...`
"""
