...
from app.db.schema.auth import KakaoLoginRequest, KakaoLoginResponse, KakaoUserResponse
...
async def kakao_login_service(payload: KakaoLoginRequest, db: AsyncSession) -> KakaoLoginResponse:
    try:
...
async def kakao_signup_service(payload: KakaoLoginRequest, db: AsyncSession) -> KakaoLoginResponse:
    try:
        # 1. Authorization code로 access_token 교환
        token_url = "https://kauth.kakao.com/oauth/token"
        token_data = {
